...
bool EventSpawner::initializeSpawnableList() {
    ...
    registerSpawnable(PC_PUSHABLE_ROCK, "Object/PushBox", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<PushBox>(fromEventMap, pos, params[0]);
    });

    registerSpawnable(PC_TRIGGER_CRATE, "Object/TriggerCrate", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<TriggerCrate>(fromEventMap, pos, params[0]);
    });

    registerSpawnable(PC_BRIDGE, "Object/BridgePiece", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<DynamicBridge>(fromEventMap, pos, params[0], static_cast<DynamicBridgeType>(params[1]), params[2]);
    });
    ...
    for (uint eventNo = static_cast<uint>(PC_SPRING_RED); eventNo <= static_cast<uint>(PC_SPRING_BLUE); ++eventNo) {
        registerSpawnable(static_cast<PCEvent>(eventNo), "Object/Spring", 
            [this, eventNo](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
            return createCommonActorEvent<Spring>(fromEventMap, pos, static_cast<SpringType>(1 + (eventNo - PC_SPRING_RED)),
                static_cast<unsigned char>(params[0]));
        });
    }

    registerSpawnable(PC_MOVING_PLATFORM, "Object/MovingPlatform", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<MovingPlatform>(fromEventMap, pos,
    ...
    registerTrivialSpawnable<TurtleShell>(PC_TURTLE_SHELL, "Enemy/TurtleShell", sf::Vector2f(0.0, 0.0));
    ...
    registerSpawnable(PC_WARP_COIN_BONUS, "Object/BonusWarp", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<BonusWarp>(fromEventMap, pos, params);
    });
    ...
    registerSpawnable(PC_LIGHT_STEADY, "Object/NoResources", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<StaticLight>(fromEventMap, pos, params[0]);
    });

    registerSpawnable(PC_LIGHT_PULSE, "Object/NoResources", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<PulsatingLight>(fromEventMap, pos, params[0], params[1], params[2]);
    });

    registerSpawnable(PC_CRATE_AMMO, "Object/CrateContainer", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<AmmoCrate>(fromEventMap, pos, params[0] < WEAPONCOUNT ? (WeaponType)params[0] : (WeaponType)0);
    });

    registerSpawnable(PC_BARREL_AMMO, "Object/BarrelContainer", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<AmmoBarrel>(fromEventMap, pos, params[0] < WEAPONCOUNT ? (WeaponType)params[0] : (WeaponType)0);
    });

    registerSpawnable(PC_CRATE_GENERAL, "Object/CrateContainer", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<CrateContainer>(fromEventMap, pos, (PCEvent)params[0], params[1]);
    });

    registerSpawnable(PC_BARREL_GENERAL, "Object/BarrelContainer", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<BarrelContainer>(fromEventMap, pos, (PCEvent)params[0], params[1]);
    });

    registerSpawnable(PC_CRATE_GEM, "Object/CrateContainer", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<GemCrate>(fromEventMap, pos, params[0], params[1], params[2], params[3]);
    });

    registerSpawnable(PC_BARREL_GEM, "Object/BarrelContainer", [this](bool fromEventMap, const CoordinatePair& pos, const quint16 params[]) {
        return createCommonActorEvent<GemBarrel>(fromEventMap, pos, params[0], params[1], params[2], params[3]);
    });
...
std::shared_ptr<CommonActor> EventSpawner::spawnEvent(bool fromEventMap, PCEvent type, const CoordinatePair& pos, const quint16 params[8]) const {
    if (spawnableEvents.contains(type)) {
        auto apiPtr = api.lock();
        if (apiPtr != nullptr) {
            return spawnableEvents.value(type).spawner(fromEventMap, pos, params);
        }
    }
...
void EventSpawner::registerTrivialSpawnable(PCEvent type, const QString& resourceName, P... params) {
    registerSpawnable(type, resourceName, [this, params...](bool fromEventMap, const CoordinatePair& pos, const quint16[]) {
        return createCommonActorEvent<T, P...>(fromEventMap, pos, *&params...);
    });
}

template<typename T, typename... P>
std::shared_ptr<CommonActor> EventSpawner::createCommonActorEvent(bool fromEventMap, const CoordinatePair& pos, P... params) {
    auto apiPtr = api.lock();
    if (apiPtr != nullptr) {
